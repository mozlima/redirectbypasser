...
function startup(startupData, startupReason) {
	...
	function onMessage(msgData, sendResponse) {
		switch (msgData.name) {
			case "opts.get":
				sendResponse(CU.cloneInto({PAGE_DATA: ((OPTS.scriptEnabled || msgData.force)? rb.PAGE_DATA : rb.PAGE_DATA_SIMPLE)}, msgData));
				break;
			case "target.process":
			...
			case "opts.set":
				...
				var sitesrules = ((msgData.sitesrulesAction == "replace")? msgData.sitesrules : (storageGet("sitesrules") || JSON.parse(JSON.stringify(rb.SITES_RULES_DEFAULT))));
	...
	function showNotification(scriptEnabled) {
		...
		try {
			notify.showAlertNotification("chrome://redirectbypasser/content/images/" + ((scriptEnabled)? "rb-icon48-enable.png" : "rb-icon48-disable.png"), "Redirect Bypasser", "");
		} catch (e) {}
	}
...
function uninstall(uninstallData, uninstallReason) {
	if (uninstallReason == ADDON_UNINSTALL) {
		CC["@mozilla.org/preferences-service;1"].getService(CI.nsIPrefService).getBranch("extensions.redirectbypasser.").deleteBranch("extensions.redirectbypasser.");
	}
}