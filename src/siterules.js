"use strict";
var SITERULE_FORMDATA;

sendMessage({name: "siterules.addform.getdata"}, function(msgData) {
	SITERULE_FORMDATA = msgData;
	mlWatcher.setItem("siteruleFormData", 1);
});

mlWatcher.onItemsDone("resources", "siteruleFormData", function() {
	...
	var eventActions = {
		...
		"change.pattern.switch": function(ev) {
			var value = ev.target.value;
			
			SITESRULES.rules.some(function(rule) {
				var stop;
				...
				rule[0].some(function(pattern) {
					if (pattern == value) {
						...
						rule[1].forEach(function(param) {
							if (!document.querySelector("input.siterule-rule-param[value=\"" + CSS.escape(param) + "\"]")) {
								addItem("siterule-rule-param").setAttribute("value", param);
							}
						});
			...
			SITESRULES.ignore.some(function(pattern) {
				if (pattern == value) {
	...
	var url							= new URL(SITERULE_FORMDATA.url || "http://example.com");
	var originWithoutScheme			= url.origin.replace(/^[^:]*/,"*");
	...
	for (var i = 1, l = paths.length; i < l; i++) {
		...
		if ((paths[i].length && (i + 1 == l) && (!paths[i + 1]))
			|| ((!paths[i].length && i > 1) || ((i + 1 == l) && (paths[i].indexOf(".") !== -1)))
		) {
			pathsArray.push(pathsStr + "*");
	...
	[originWithoutSchemeAndTld, originWithoutScheme, url.origin].forEach(function(originKey) {
		pathsArray.forEach(function(pathsKey) {
			if ((originKey != originWithoutSchemeAndTld) || (pathsKey != "*")) {
				selectSiterulePattern.options.add(new Option(originKey  + pathsKey, originKey  + pathsKey));
			}
		});
	});
	
	if (SITERULE_FORMDATA.info) {
		addItem("siterule-rule-param").value = SITERULE_FORMDATA.info.join(",");
	}
	...
	SITESRULES.rules.some(function(rule) {
		...
		rule[0].some(function(pattern) {
			if (selectSiterulePattern.querySelector("[value=\"" + CSS.escape(pattern) + "\"]")) {
	...
	function getFormData() {
		var sitesrules = JSON.parse(JSON.stringify(SITESRULES));
